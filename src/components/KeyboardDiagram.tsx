...
export default function KeyboardDiagram({ activeKeys, typedKeys = [], normalKeys = [], layout }: KeyboardDiagramProps) {
  ...
  const getKeyStyle = (key: string) => {
    if (activeKeys.includes(key)) {
      return "bg-green-400 text-black border-green-300 shadow-[0_3px_0_#16a34a] font-bold scale-110";
    }
    ...
    <div className="flex flex-col items-center gap-1 my-3 select-none">
      ...
      {(activeKeys.length > 0 || normalKeys.length > 0) && (
        <div className="flex gap-3 mt-1 text-[9px] text-zinc-500">
          {activeKeys.length > 0 && <span className="flex items-center gap-1"><span className="w-2 h-2 bg-green-400 rounded-sm inline-block" />AZIK</span>}
          {normalKeys.length > 0 && <span className="flex items-center gap-1"><span className="w-2 h-2 bg-amber-400 rounded-sm inline-block" />通常ローマ字</span>}
        </div>
      )}
    </div>
  );
