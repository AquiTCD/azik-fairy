...
1. **レベル別の段階的ステージ構成**
   - AZIKの導入レベル（互換キー）から、二重母音短縮、特殊キー拡張（こと/もの/する/です/ます）、さらには外来語拡張まで、順序立てて学べるステージを用意しています。各ステージには星評価と自己ベストが記録されます。

2. **2種類のゲームモード**
   - **TRAINING（練習）**: AZIK以外の通常ローマ字入力もミスなく受け付ける自由練習モード。
   - **CHALLENGE（挑戦）**: AZIKショートカットが使える箇所では通常入力を強制シャットアウトする本番モード。AZIKを指に叩き込むためのスパルタ仕様です。

3. **タイムアタックモード**
   - 60秒間に何文字打てるかを競うモードです。総合的な語彙から出題され、純粋な入力スピードを計測します。

4. **ゴーストレース**
   - 過去の自己ベストWPM（幽霊）と並走するレース機能。プログレスバー上に幽霊の進捗が表示され、自分との差がリアルタイムで分かります。

5. **弱点練習**
   - 練習中に入力ミスが多かった仮名パターンを自動的に記録・分析し、苦手な組み合わせだけを集中的に練習できるモードを自動生成します。

6. **リアルタイムAZIK率分析**
   - 打鍵を監視し、どれだけAZIKの短縮キーを活用できたかを「AZIK度 (AZIK Ratio)」および「節約キー数 (SAVED KEYS)」としてリアルタイムで算出・表示します。

7. **成長グラフ（STATS）**
   - 日付ごとのWPM・精度・AZIK率の推移をグラフで確認できます。7日/30日/90日の期間切り替えと、先週比WPMの比較表示に対応しています。

8. **キーヒートマップ**
   - リザルト画面で、ミスが多かったキーをキーボード図上にヒートマップ表示します。どのキーに集中して練習すべきかが一目で分かります。

9. **自由なキーカスタマイズ & 設定**
   - JIS/USキーボード配列の切り替え、撥音（ん）・促音（っ）・長音（ー）のキー変更に対応。`kana-rule.conf` / TSV / JSON の一括インポートも可能です。

10. **サウンドテーマ**
    - `soft`（柔らかい）、`8bit`（レトロゲーム風）、`typewriter`（タイプライター風）の3種類のサウンドテーマを選べます。OFFにすることも可能です。

11. **豊富な語彙とインポート機能**
    - 標準的な例文に加え、Mozc OSS辞書やSKK辞書からインポートされた数万語規模の本格的な語彙ステージを搭載しています。

12. **X (Twitter) シェア & OGP中継**
